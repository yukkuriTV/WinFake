...
Launcher.exe（32bitアプリケーションであると事前に分かっている場合はInjectorx86.exe）を起動します。上下矢印キーで［通常の起動］か［対象プロセスを監視して起動］を選ぶことができます。

［通常の起動］

ファイル選択ダイアログが表示されるので、そこからフックしたいアプリを選択して実行します。フックは起動前に確実に注入されますが、起動後にアプリがフックを検出して置き換える機能を持っていた場合、機能しません。この場合は【高度な使い方】を参照してください。

［対象プロセスを監視して起動］

フックしたいアプリの.exeまでを含めたファイル名を入力すると、WinFakeはプロセスへの監視を開始し、対象のアプリが見つかると自動的にフックを開始します。複数プロセスの起動に備え、フックは実行中、繰り返し行われます。これはWinFake 2.0まで存在した常駐フック機能に近い動作をします。

【32bitサポート】

Launcher.exeで32bitアプリケーションを選択すると、［通常の起動］であれば即時に、［対象プロセスを監視して起動］であれば当該アプリケーションが実行された時に、自動的にInjectorx86.exeが起動します。ただし、開発段階で複数の問題が発生したため、自動で32bitアプリケーション用のフックを行う機能は提供していません。そのため、これが発生した場合は、一度フックしたいアプリを閉じて、既に起動しているInjectorx86.exeで再度フックを行う必要があります。Launcher.exeを管理者権限で起動すると、Injectorx86.exeも管理者権限で実行できます。

【高度な使い方】

このソフトは工夫次第で、APIフックに一定の耐性を持つソフトウェアの耐性を突破することができます。以下にその方法の一例をご紹介します。

フック対象のアプリを仮に「API.exe」（D/API.exe）とします。
